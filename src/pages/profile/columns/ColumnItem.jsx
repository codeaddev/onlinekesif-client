...
function ColumnItem({ item }) {
  ...
  return (
    <div onClick={() => navigate("kesiflerim")} className="col-item">
      <div className="col-top">
        <div className="col-top-top">
          <p>
            {item.wishDetail.MeskenTipi.a}-{item?.adress?.substring(0, 15)}
            {item?.adress?.length > 15 ? "..." : null}
          </p>
          <div className="col-wish-indicator">{item?.mainWish}</div>
        </div>
        <div className="col-top-bottom">
          <p className="summary">{item?.wishDetail.summary?.a}</p>
        </div>
      </div>
      <div className="col-bottom">
        ...
        <div className="col-bottom-right">
          ...
          <Moment
            className="moment"
            to={new Date(item?.publishRemaining.seconds * 1000)}
          >
            {new Date()}
          </Moment>
        </div>
