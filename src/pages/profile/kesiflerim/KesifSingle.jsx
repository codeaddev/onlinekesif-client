import { Button, Fade, Hidden, Tooltip } from '@mui/material'
...
function Row({row}) {
  ...
  const handleCancel=async(e,row)=>{
    ...
    await updateDoc(doc(db,"Jobs",state.doc),{
      statue:4,
      statueMap:{id:new Date().valueOf(),createdAt:new Date(),what:4,for:row.firm,who:auth?.currentUser?.uid},
  ...
  const handleComplete=async(e)=>{
    ...
    await updateDoc(doc(db,"Jobs",state.doc),{
      ...
      statue:6,
      statueMap:{id:new Date().valueOf(),createdAt:new Date(),what:6,for:row.firm,who:auth?.currentUser?.uid}
    })
    .then(()=>{
      setAlertMessage({
        visible:true,
...
const handleRefuse=async(e,row)=>{
  ...
  const handleCancelAlert=async(e,row)=>{
    ...
    setAlertMessage({
      ...
      isError:false,
      title:"Uyarı",
      infoText:"Servis Talebinizi İptal Etmek Üzeresiniz. Bu işlem geri alınamaz!",
  ...
  const handleRefuseAlert=async(e,row)=>{
    ...
    setAlertMessage({
      ...
      isError:false,
      title:"Uyarı",
      infoText:"Firma Teklifini Reddetmek Üzeresiniz. Bu işlem geri alınamaz!",
  ...
  let newArray=state.Offers.filter(i=>i.id!==row.id)
    ...
    var otherPrices=othersList.map(i=>{return {products:i.relatedProducts,firmName:i.firmName }})
  ...
  return (
    ...
    <div className='body-row'>
      ...
      sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell className='row-cell first' align="left">{row.firmName}</TableCell>
        <TableCell className='row-cell first' align="right">
          <img src={MiniKpu} alt=''/>
           {row.KPU} Puan</TableCell>
        <TableCell  className='row-cell first' align="right">Teklif: {TLLocale.format(row.totalPrice)} ₺</TableCell>
        <TableCell>
          <IconButton
              ...
              <table>
                <tr>
                  <th className='index'>S.No</th>
                    ...
                    <th  className='price'align="right">Birim Fiyat (₺)</th>
                    <th  className='price'align="right">Toplam Fiyat (₺)</th>
                </tr>
                {row.relatedProducts.map((historyRow,index) => {
                  ...
                  <p>Bu bir ön keşif formudur. Uygulama esnasında ek maliyetler çıkabilir. </p>
                    ...
                    {state.statue!==6&&<div className="button-area">
                    <button 
                      onClick={(e)=>handleCancelAlert(e,row)}
                      className="refuse">İşi İptal Et</button>
                      {
                      state.statue!==3&&
                      <button 
                      onClick={(e)=>handleRefuseAlert(e,row)}
                      className="refuse">Teklifi Reddet</button>}
                      {
                      state.statue!==3&&
                      <button 
                      onClick={(e)=>handleAcceptAlert(e,row)}
                      className="accept">Teklifi Kabul Et</button>}
                      {
                      state.statue===3&&
                      <button 
                      onClick={handleCompleteAlert}
                      className="accept">İş Tamamlandı</button>}
...
const handleCancel=async(e)=>{
  ...
  await updateDoc(doc(db,"Jobs",state.doc),{
      statue:5,
      cancelled:true,
      statueMap:arrayUnion({id:new Date().valueOf(),for:"all",refused:true,createdAt:new Date()})
    })
    .then(()=>setUpdating(false))
    .then(()=>
    setAlertMessage({
    ...
    <div className='kesif'>
      <div className="kesif-container">
        ...
        type="button" className="collapsible">Talep Detaylarınız<ExpandMoreIcon/></button>
          ...
          <>
          <div className='empty-case'>{state.adminned?<p>Henüz Teklif Yapılmamıştır</p>:<p>Henüz Keşif Oluşturulmamıştır</p>}
          </div>
          <button
          onClick={handleCancelAlert}
          className='button cancel'
          >Teklifi İptal Et</button>
          </>
          }
