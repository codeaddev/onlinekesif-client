...
function Kesiflerim({ setMainList }) {
  ...
  const headerList = [
    { id: "01", label: "Tüm Teklifler", data: myJobs },
    {
      id: "02",
      label: "Keşif Aşaması",
      data: myJobs.filter((i) => !i.statue === 0),
    },
    {
      id: "03",
      label: "Teklif Aşamasında",
      data: myJobs.filter((i) => i.statue !== 0 && i.statue !== 6),
    },
    { id: "04", label: "Tamamlandı", data: myJobs.filter((i) => i.completed) },
    {
      id: "05",
      label: "Başarısız",
      data: myJobs.filter((i) => i.expired || i.statue === 4 || i.statue === 5),
    },
  ];
  const style = {
    position: "absolute",
  ...
  const columns = [
...

    {
      ...
      flex: 0.7,
      renderHeader: () => {
        return <div>Keşif Kategorisi</div>;
      },
    },
    {
      field: "adress",
      ...
      flex: 1,
      renderHeader: () => {
        return <div>Adres Bilgileri</div>;
      },
      valueGetter: (params) =>
    ...
    },
    {
      ...
      description:
        "Teklif yapan firmalar arasında en çok ve en az fiyatı gösterir. 1 teklif varsa yalnızca o teklif fiyatını gösrerir.",
      sortable: false,
      width: 100,
      headerAlign: "center",
      headerClassName: "column-header",
      ...
      flex: 1,
      renderHeader: () => {
        return <div>Teklif Fiyatı</div>;
      },
      renderCell: (props) => {
        let TLLocale = Intl.NumberFormat("tr-TR");
        var array = props?.row.Offers?.map((i) => i.totalPrice);
        return (
          <div className="offered-prices-list">
            {props.row.Offers.length > 0 && props.row.Offers.length < 3 ? (
              <span className="offered-prices">
                {TLLocale.format(Math.min(...array))}₺ |{" "}
                {TLLocale.format(Math.max(...array))}₺
              </span>
    ...
    },
    {
      ...
      align: "center",
      renderHeader: () => {
        return <div>Teklif Sayısı</div>;
      },
      flex: 0.6,
      valueGetter: (params) => `${params.row?.Offers.length}`,
    },
    {
      field: " ",
      headerName: " ",
  ...
  const columnsMobile = [
...

    {
      ...
      renderCell: (props) => {
        return (
          <div>
            {props.row.Offers.length > 0 ? (
              props.row.Offers.map((i) => {
                return (
                  <span className="offered-prices mobile" key={i.id}>
                    {TLLocale.format(i.totalPrice)} ₺
                  </span>
                );
              })
            ) : (
        ...
        <div className="kesiflerim-inner-container">
          ...
          >
            <DataGrid
              ...
              columns={
                isMobile | (window.innerWidth < 800) ? columnsMobile : columns
              }
              pageSize={7}
              getRowId={(row) => row.id}
              pagination
