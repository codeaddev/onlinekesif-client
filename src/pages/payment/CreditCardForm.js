...
const CreditCardForm = ({state}) => {
  const { handleChange, handleFocus, handleSubmit, values, errors,user,card,product,
    handleChangeUser,handleChangeCard, handleChangeProduct, SKT,handleChangeSKT, handleChangeInstallment,installment} = useForm();
    var money=100
    const taksits=[
...
const CheckedTarget=()=>{
  return(
    ...
    href="http://localhost:3001/KVKK" target={"_blank"}>Mesafeli Satış Sözleşmesini</a> okudum anladım.</div>
  )
}
